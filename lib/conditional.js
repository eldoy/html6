...
function conditional(node, opt) {
  ...
  while (current) {
    ...
    // Recursively process children conditionals
    if (current.children && current.children.length) {
      for (var i = 0; i < current.children.length; i++) {
        var child = current.children[i]

        if (child.type === 'element') {
          // Check if it has a conditional attribute
          var childAttrs = child.attributes || []
          var hasConditionalAttr = childAttrs.some((attr) => keys[attr.key])

          if (hasConditionalAttr) {
            conditional(child, opt) // process the child
            // Replace the element with its masked content
            current.children[i] = {
              type: 'text',
